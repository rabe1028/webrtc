...
impl RtcpSenderReportPacket {
    pub fn get_length(&self) -> u32 {
        4 + self.sender_info.get_length()
            + self.reports.len() as u32 * RtcpReportBlock::get_length()
    }
    ...
    pub fn from_bytes(bytes: &mut octets::Octets, count: u8) -> Result<RtcpSenderReportPacket> {
        if bytes.len()
            != RTCP_HEADER_LENGTH + RTCP_SR_INFO_LENGTH + RTCP_REPORT_BLOCK_LENGTH * count as usize
        {
